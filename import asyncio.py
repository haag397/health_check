...
async def send_request(client, log_list, unique_invoice):
    ...
    if response.status_code != 200:
        raise Exception(f"Error: Received status code {response.status_code} for invoice number {unique_invoice}")
...
async def load_test(requests_per_minute, duration_minutes=1):
    ...
    try:
        ...
        print(f"Test stopped due to error: {e}")
    
    # Save logs after test is completed or stopped
